...
import { nameValidation } from "../../utils/validation.ts";
...
const ModalPreRegister: React.FC<ModalPreRegisterProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  const validateName = (value: string) => {
    ...
    if (!nameRegex.test(value)) {
      setNameError(
        "Name must be 3-16 characters long, no spaces, only letters"
      );
    } else {
      setNameError("");
    }
      ...
      >
        ...
        >
          <Typography variant="h3" align="center">
            PRE-REGISTER FOR BETATEST ACCESS
          </Typography>
        </Box>
        <Box
          sx={{
        ...
        >
          <FormControlLabel
            ...
            label={
              <Box sx={{ marginLeft: 2 }}>I am 18 years of age or older</Box>
            }
          />
          <FormControlLabel
            sx={{ m: 1 }}
            control={
              <Checkbox
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setIsSendEmailAccepted(e.target.checked)
                }
                color="primary"
                id="terms"
                name="terms"
            ...
            label={
              <Box sx={{ marginLeft: 2 }}>
                I agree to receive NeoXonline emails (e.g. betatest invite,
                NeoXonline news)
              </Box>
            }
          />
