...
const Keep = () => {
   const [arrNotes, setArrNotes] = useState<Array<noteType>>([{
      id: 1,
      text: 'some text',
      titleNote: '',
      img: null,
   ...
   const [todolists, setTodolists] = useState<TodolistType[]>(
      // []as TodolistType[])
     [ {id: 'j123', title: 'What to learn', filter: 'none', background:"#fff"},
      {id: "v1()", title: 'What to buy', filter: 'none', background:"#fff"},
   ])
   ...
   const addTodolist = () => {
      ...
      const newTodolist: TodolistType = {id: todolistId, title: title, filter:"none",background:noteBackgroundColor}
   ...
   const updateTodolist = (todolistId: string, title: string) => {
      ...
      console.log("updateTodolist title" ,title);
      console.log("updateTodolist" ,newTodolists[1]);
         ...
         <Box>
            <Paper square={false} pt={2} pb={2} className={cl.paper} style={{backgroundColor: noteBackgroundColor}}
               ...
               <NoteCreationPanel isOpen={isOpen} isTodoList={isTodoList} handleFocus={handleFocus}
                                  ...
                                  newNoteTitle={newNoteTitle} newNote={newNote} updateTodolist={updateTodolist}
                                  updateTask={updateTask} addTask={addTask}
                                  removeTask={removeTask} changeTodoFilter={changeFilter}
                                  changeTaskStatus={changeTaskStatus} removeTodolist={removeTodolist} todolistId={todolists[0].id}
                                  tasks={tasks[todolists[0].id]}
               />: <p>loading ... </p>
               }
               {isOpen &&
                   <InvitationBoxFooter setText={setText} getBackground={backgroundHandler} getImg={inputImgHandler}
